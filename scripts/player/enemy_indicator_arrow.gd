...
func  _ready() -> void:
	modulate = Color.from_hsv(randf(), 0.65, 0.75)
...
func _process(delta) -> void:
	...
	position = screen_edge_position - Vector2(40, 40)
	rotation = direction.angle()


func get_screen_edge_position(direction: Vector2, margin: int = 50) -> Vector2:
	# get screen rect Bounds from direction angle
	var screen_rect: Rect2 = Rect2(Vector2.ZERO, get_parent().get_viewport().get_visible_rect().size)
	var screen_center: Vector2 = screen_rect.get_center()

	# Define a line segment starting from the center and extendiA  ng far beyond the screen edge
